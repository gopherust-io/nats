...
Baseline is `DefaultConfig()` plus job-worker knobs (worker pool, `BackpressureNak`). Stream topology is created explicitly in the example (WorkQueue + memory + Replicas:1 for the lab). Local demo overrides only:

| Setting | Value | Why |
|---------|-------|-----|
| Stream `Replicas` / `Storage` | `1` / memory | Single local server; production: FileStorage + Replicas:3 |
| `DuplicateWindow` | 2m | Publish Msg-Id dedup window |
| Worker pool | enabled (8 / 256) | Push throughput; keep until metrics say otherwise |
...
| Backpressure | `BackpressureNak`, MaxAckPending 1000 | Bursty job queues |
...
| Setting | Value | Why |
|---------|-------|-----|
| Baseline | `DefaultConfig()` + stream WorkQueue | Pull path does not need the push worker pool |
...
|-------|-------------|---------------|
| Baseline | `DefaultConfig()` + job-worker knobs | `DefaultConfig()` + pull durable |
